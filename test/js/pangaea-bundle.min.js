...
if(pan.settings.autoResize){onFullscreenChange=function(){var c=document.getElementById('canvas');if(document.fullscreen||document.mozFullScreen||document.webkitIsFullScreen){pan.settings.tempSize={w:pan.canvas.width,h:pan.canvas.height};pan.canvas.width=screen.width;pan.canvas.height=screen.height;}else{if(pan.settings.tempSize){pan.canvas.width=pan.settings.tempSize.w;pan.canvas.height=pan.settings.tempSize.h;}}
c.width=pan.canvas.width;c.height=pan.canvas.height;};document.addEventListener('fullscreenchange',onFullscreenChange);document.addEventListener('mozfullscreenchange',onFullscreenChange);document.addEventListener('webkitfullscreenchange',onFullscreenChange);}}());(function(){'use strict';pan.Layer=function(data,name,type,left,top,width,height,objects){var map=pan.canvas.map;this.data=data||[];this.name=name;this.type=type;this.left=left||0;this.top=top||0;this.width=width||map.width;this.height=height||map.height;this.coords=[];this.objects=objects||[];};pan.Layer.prototype.update=function(){};pan.Layer.prototype.render=function(){};pan.Layer.prototype.prerender=function(){var map=pan.canvas.map,i,x,y,tileIndex,tileset,atlas,atlasindex,frame,length;if(this.type==='tilelayer'){for(i=0;i<this.data.length;i++){tileIndex=this.data[i];if(tileIndex===0){continue;}
for(x=pan.canvas.tilesets.length;x>=0;x){x--;if(pan.canvas.tilesets[x].firstgid<=tileIndex){tileset=pan.canvas.tilesets[x];x=-1;}}
for(y=0;y<pan.canvas.atlases.length;y++){atlas=pan.canvas.atlases[y];atlasindex=y;frame=this.selectFrame(atlas,tileset.image);if(frame){break;}}
this.computeFrame(map,frame,i,tileset,tileIndex);if(!pan.canvas.table){length=Math.ceil((this.width/map.tilewidth)*(this.height/map.tileheight));pan.canvas.table=[length];}
pan.canvas.table[tileIndex]={aindex:0,srcx:frame.srcx,srcy:frame.srcy,w:tileset.tilewidth,h:tileset.tileheight};}}else{if(this.name==='lights'){this.type='lights';for(i=0;i<this.objects.length;i++){frame=this.objects[i];if(frame.ellipse&&frame.visible){pan.canvas.lights.push({x:frame.x,y:frame.y,w:frame.width,h:frame.height});}}}else if(this.name==='hittest'){this.type='hittest';for(i=0;i<this.objects.length;i++){frame=this.objects[i];if(frame.visible){pan.canvas.hitRegions.push({x:frame.x,y:frame.y,w:frame.width,h:frame.height});}}}}};pan.Layer.prototype.computeFrame=function(map,frame,index,tileset,tileIndex){var xpos,ypos,localindex,localwidth;frame.srcx=0;frame.srcy=0;localindex=tileIndex-tileset.firstgid;localwidth=frame.frame.w/map.tilewidth;frame.srcx=Math.round(((localindex/localwidth)%1)*localwidth)*map.tilewidth;frame.srcx=frame.srcx+frame.frame.x;frame.srcy=Math.floor(localindex/localwidth)*map.tileheight;frame.srcy=frame.srcy+frame.frame.y;xpos=Math.round(((index/map.width)%1)*map.width*map.tilewidth);ypos=Math.floor(index/map.width)*map.tileheight;this.coords[index]={xpos:xpos,ypos:ypos};};pan.Layer.prototype.selectFrame=function(atlas,key){var z;for(z=0;z<atlas.frames.length;z++){if(atlas.frames[z].filename===key){return atlas.frames[z];}}
...
pan.canvas.map.tilewidth=json.tilewidth;pan.canvas.map.tileheight=json.tileheight;pan.canvas.map.width=json.width;pan.canvas.map.height=json.height;pan.canvas.map.clamp.x=pan.canvas.width/2;pan.canvas.map.clamp.y=pan.canvas.height/2;pan.canvas.map.clamp.w=(pan.canvas.map.width*pan.canvas.map.tilewidth)-pan.canvas.map.clamp.x;pan.canvas.map.clamp.h=(pan.canvas.map.height*pan.canvas.map.tileheight)-pan.canvas.map.clamp.y;for(i=0;i<json.layers.length;i++){pan.canvas.layers.push(new pan.Layer(json.layers[i].data,json.layers[i].name,json.layers[i].type,json.layers[i].x,json.layers[i].y,json.layers[i].width,json.layers[i].height,json.layers[i].objects));}
...
if(renderCallback){pan.canvas.onrender=renderCallback;}},start:function(){pan.canvas.prerender();if(pan.util){pan.util.start(pan.canvas);}
if(!pan.canvas.frameId){pan.canvas.frame();}},stop:function(){pan.canvas.ready=false;},reset:function(){pan.canvas.stop();pan.canvas.layers=[];pan.canvas.atlases=[];pan.canvas.tilesets=[];pan.canvas.spritesheets=[];pan.canvas.table=undefined;pan.canvas.lights=[];pan.canvas.hitRegions=[];pan.canvas.map={tileheight:0,tilewidth:0,width:0,height:0,offsetx:0,offsety:0,clamp:{x:0,y:0,w:0,h:0}};if(pan.util){pan.util.reset(pan.canvas);}},frame:function(){if(pan.canvas.ready){pan.canvas.update();pan.canvas.render();}
pan.canvas.frameId=window.requestAnimationFrame(pan.canvas.frame);},update:function(){var i;for(i=0;i<pan.canvas.layers.length;i++){pan.canvas.layers[i].update();}
if(pan.canvas.onupdate){pan.canvas.onupdate(pan.canvas);}
if(pan.util){pan.util.update(pan.canvas);}},prerender:function(){var i;for(i=0;i<pan.canvas.layers.length;i++){pan.canvas.layers[i].prerender();}
pan.canvas.ready=true;},render:function(){var x,i,layer,tileIndex,record,coords,xpos,ypos,width=pan.canvas.width,height=pan.canvas.height,offsetx=pan.canvas.map.offsetx,offsety=pan.canvas.map.offsety;pan.canvas.clear();for(x=0;x<pan.canvas.layers.length;x++){layer=pan.canvas.layers[x]||{};if(layer.type!=='tilelayer'){continue;}
for(i=0;i<layer.data.length;i++){tileIndex=layer.data[i];coords=layer.coords[i];if(tileIndex===0||!coords){continue;}
record=pan.canvas.table[tileIndex];xpos=coords.xpos+offsetx;ypos=coords.ypos+offsety;if(xpos+record.w>offsetx&&xpos<width&&ypos+record.h>offsety&&ypos<height){pan.canvas.context.drawImage(pan.canvas.atlases[record.aindex].image,record.srcx,record.srcy,record.w,record.h,xpos,ypos,record.w,record.h);}}}
if(pan.canvas.onrender){pan.canvas.onrender();}
if(pan.util){pan.util.render(pan.canvas);}},clear:function(){pan.canvas.context.fillStyle=pan.canvas.backcolor;pan.canvas.context.fillRect(0,0,pan.canvas.width,pan.canvas.height);}};pan.canvas.print=function(text,x,y,font,color){x=x||12;y=y||16;pan.canvas.context.font=font||'10pt "Courier New", Courier, monospace';pan.canvas.context.fillStyle=color||'cyan';pan.canvas.context.fillText(text,x,y);};pan.canvas.drawHitRegions=function(borderColor,fillColor){var i,r,bc=borderColor||'red',fc=fillColor||'rgba(255, 0, 0, 0.1)';pan.canvas.context.beginPath();for(i=0;i<pan.canvas.hitRegions.length;i++){r=pan.canvas.hitRegions[i];pan.canvas.context.rect(r.x+pan.canvas.map.offsetx,r.y+pan.canvas.map.offsety,r.w,r.h);}
pan.canvas.context.fillStyle=fc;pan.canvas.context.fill();pan.canvas.context.strokeStyle=bc;pan.canvas.context.stroke();pan.canvas.context.closePath();};}());(function(){'use strict';pan.util=pan.util||{};pan.util.start=function(canvas){canvas.last=Date.now();if(pan.settings.drawFps||pan.settings.debugInit){canvas.deltaTimer=new pan.util.DeltaTimer();canvas.deltaTimer.start(60);}
...
if(pan.settings.drawFps){canvas.deltaTimer.ready();}};pan.util.render=function(canvas){if(pan.settings.drawHitRegions){if(pan.canvas.drawHitRegions){pan.canvas.drawHitRegions();}}
if(pan.settings.enablePlayer){canvas.player.draw(canvas.context);}
if(pan.settings.drawFps){canvas.context.font=pan.settings.fontStyle;canvas.context.fillStyle=pan.settings.fontColor;canvas.context.fillText('FPS: '+
...
return true;}
return false;},getFrameRate:function(){return Math.round(frameRate*1000,10)/1000;}};};pan.util.Player=function(x,y,w,h){var c,bc,cx,cy;x=x||0;y=y||0;w=w||32;h=h||32;cx=x;cy=y;c='rgba(0, 98, 174, 0.75)';bc='rgba(255, 64, 64, 1)';return{color:c,bordercolor:bc,speed:2,update:function(){var key,rate,bounds,i,region,buffer={x:cx,y:cy};if(pan.util.keyboard.stack.length>0){rate=pan.util.keyboard.shift?this.speed*2:this.speed;key=pan.util.keyboard.peek();if(key==='w'){cy-=rate;}else if(key==='a'){cx-=rate;}else if(key==='s'){cy+=rate;}else if(key==='d'){cx+=rate;}
if(cx===buffer.x&&cy===buffer.y){return;}
cx=cx.clamp(0,(pan.canvas.map.width*32)-w);cy=cy.clamp(0,(pan.canvas.map.height*32)-h);if(pan.settings.enableHitTesting){for(i=0;i<pan.canvas.hitRegions.length;i++){region=pan.canvas.hitRegions[i];if(this.hitTest(region,cx,cy,cx+w,cy+h)){cx=buffer.x;cy=buffer.y;return false;}}}
bounds=pan.canvas.map.clamp;if(cx>=bounds.x&&cx<=bounds.w){pan.canvas.map.offsetx=-(cx-bounds.x);x=bounds.x;}else{x=(cx>=bounds.x)?cx-(bounds.w-bounds.x):cx;x=x.clamp(0,pan.canvas.width-w);}
if(cy>=bounds.y&&cy<=bounds.h){pan.canvas.map.offsety=-(cy-bounds.y);y=bounds.y;}else{y=(cy>=bounds.y)?cy-(bounds.h-bounds.y):cy;y=y.clamp(0,pan.canvas.height-h);}}},draw:function(context){context.beginPath();context.rect(x,y,w,h);context.strokeStyle=this.bordercolor;context.stroke();context.fillStyle=this.color;context.fill();context.closePath();},hitTest:function(region,left,top,right,bottom){return(region.x<right)&&((region.x+region.w)>left)&&(region.y<bottom)&&((region.y+region.h)>top);}};};pan.util.keyboard={stack:[],shift:false,peek:function(){var temp=this.stack.pop();this.stack.push(temp);return temp;},keydown:function(code){if(this.stack.length>0){if(this.peek()!==code){this.stack.push(code);}}else{this.stack.push(code);}},keyup:function(code){this.stack=this.stack.filter(function(item){return item!==code;});},clear:function(){this.stack=[];this.shift=false;},bind:function(){document.onkeydown=function(event){event=event||window.event;var code=event.keyCode;if(code===16){pan.util.keyboard.shift=true;}else if(code===87){pan.util.keyboard.keydown('w');}else if(code===65){pan.util.keyboard.keydown('a');}else if(code===83){pan.util.keyboard.keydown('s');}else if(code===68){pan.util.keyboard.keydown('d');}
...
pan.util.keyboard.toString());}};window.onblur=function(){pan.util.keyboard.clear();};},toString:function(){return'[stack.length:'+this.stack.length+',shift:'+this.shift+']';}};Number.prototype.clamp=function(min,max){return Math.min(Math.max(this,min),max);};}());